document.getElementById('health-eval-form').addEventListener('submit', async function(e) {
    e.preventDefault();
    console.log('Health evaluation form submitted');
    ...
    if (!input) {
        ...
        resultDiv.innerHTML = '<span style="color:#F26522;">Please enter foods to evaluate.</span>';
        return;
    }
    
    console.log('Starting health evaluation...');
    resultDiv.innerHTML = '<div style="text-align:center;"><i class="fa fa-spinner fa-spin" style="color:#F26522;font-size:1.2rem;"></i><br><span style="color:#F26522;font-weight:bold;">Evaluating health impact...</span></div>';
    ...
    console.log('Sending request to backend with:', { foods: input, profile });
    
    try {
        const response = await fetch('/api/health-evaluation', {
            method: 'POST',
            ...
            resultDiv.innerHTML = `
                <div style="background:#f8f9fa;border-radius:12px;padding:16px;margin-bottom:12px;max-height:350px;overflow-y:auto;">
                    <h4 style="margin:0 0 8px 0;color:#1a2340;font-size:1.1rem;">
                        <i class="fa fa-heartbeat" style="color:#28a745;margin-right:8px;"></i>
        ...
        resultDiv.innerHTML = `
            <div style="background:#f8d7da;border:1px solid #f5c6cb;border-radius:12px;padding:16px;">
                ...
                <span style="color:#721c24;">Error evaluating health impact. Please try again later.</span>
            </div>
        `;
    }
});
