document.getElementById('alternatives-form').addEventListener('submit', async function(e) {
    e.preventDefault();
    console.log('Alternatives form submitted');
    ...
    if (!input) {
        ...
        resultDiv.innerHTML = '<span style="color:#F26522;">Please enter foods to find alternatives for.</span>';
        return;
    }
    
    console.log('Starting alternatives search...');
    resultDiv.innerHTML = '<div style="text-align:center;"><i class="fa fa-spinner fa-spin" style="color:#F26522;font-size:1.2rem;"></i><br><span style="color:#F26522;font-weight:bold;">Finding healthier alternatives...</span></div>';
    ...
    try {
        const response = await fetch('/api/healthier-alternatives', {
            method: 'POST',
        ...
        resultDiv.innerHTML = `
            <div style="background:#f8d7da;border:1px solid #f5c6cb;border-radius:12px;padding:16px;">
                ...
                <span style="color:#721c24;">Error finding alternatives. Please try again later.</span>
            </div>
        `;
    }
});
